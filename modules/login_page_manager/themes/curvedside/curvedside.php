...
class Plus_admin_login_theme_Curvedside extends Plus_admin_Login_Theme{

	public function __construct(){
		parent::__construct();

        $this->theme_name   = 'curvedside';
		$this->theme_prefix = 'plus_theme-'.$this->theme_name;

		$this->settings = (object) array(
			'show_outside_logo'		=> true,
	...
	public function get_settings(){
		...
		$settings 		= array(
			...
			'fields'		=> array(
                array(
					'type'			=> 'subheading',
					'content'		=> __('CurvedSide Theme Settings'),
				),
				array(
				...
				array(
                    ...
                    'default'       => array(
						'external_image'	=> $theme_path . '/curvedside-bg.jpg',
                        'repeat'     		=> 'no-repeat',
                        'position'   		=> 'center center',
						'attachment' 		=> 'fixed',
						'size'				=> 'cover',
				...
				array(
					...
					'type'			=> 'select',
					'title'			=> __('Curved Side Position','plus_admin'),
				...
				array(
                    ...
                    'type'          => 'background',
                    'title'         => __('Curved Side Background Image','plus_admin'),
	...
	public function parse_settings($settings){
		...
		$svg_color 				= ($option('login_form_bg')) ? $option('login_form_bg') : 'white';
		$svg 					= base64_encode('<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 47.2 504" style="enable-background:new 0 0 47.2 504;" xml:space="preserve"><path id="XMLID_1_" class="st0" fill="'.$svg_color.'" d="M47.2,504H0c0,0,29.3-97,29.3-252C29.3,104.3,0,0,0,0h47.2V504z"/></svg>');
		$side_position_bg 		= "top right no-repeat url(data:image/svg+xml;base64,{$svg})";
		$side_position_order 	= false;
		$side_position_right 	= '-1px';
		$side_position_left 	= false;
		
		if ($side_position == 'right'){
			$svg 					= base64_encode('<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 47.2 504" style="enable-background:new 0 0 47.2 504;" xml:space="preserve"><path id="XMLID_1_" class="st0" fill="'.$svg_color.'" d="M0,0l47.2,0c0,0-29.3,97-29.3,252c0,147.7,29.3,252,29.3,252H0L0,0z"/></svg>');
			$side_position_bg 		= "top left no-repeat url(data:image/svg+xml;base64,{$svg})";
			$side_position_order 	= 'row-reverse';
			$side_position_right 	= false;
			$side_position_left 	= '-1px';
		}
		...
		:root{
			...
			%s_login-side-position-background:		$side_position_bg;
			%s_login-side-position-order:			$side_position_order;
			%s_login-side-position:					$side_position;
			%s_login-side-position-left:			$side_position_left;
			%s_login-side-position-right:			$side_position_right;
	...
	function enqueue_scripts(){
		...
		wp_enqueue_style('cs-plus-admin-google-fonts','https://fonts.googleapis.com/css?family=Open+Sans:300,400,700', false ); 
	...
	function render_form_html(){
		...
		$register 		= $this->get_link_register('You don\'t have an account yet?');
		echo "
