...
if (!class_exists('PostType')){
	class PostType{
		...
		public function labels($labels = [])
		{
			...
			// default labels.
			$defaults = [
				'name' => sprintf(esc_attr__('%s', $this->textdomain), $this->plural),
				'singular_name' => sprintf(esc_attr__('%s', $this->textdomain), $this->singular),
				'menu_name' => sprintf(esc_attr__('%s', $this->textdomain), $this->plural),
				'all_items' => sprintf(esc_attr__('%s', $this->textdomain), $this->plural),
				'add_new' => esc_attr__('Add New', $this->textdomain),
				'add_new_item' => sprintf(esc_attr__('Add New %s', $this->textdomain), $this->singular),
				'edit_item' => sprintf(esc_attr__('Edit %s', $this->textdomain), $this->singular),
				'new_item' => sprintf(esc_attr__('New %s', $this->textdomain), $this->singular),
				'view_item' => sprintf(esc_attr__('View %s', $this->textdomain), $this->singular),
				'search_items' => sprintf(esc_attr__('Search %s', $this->textdomain), $this->plural),
				'not_found' => sprintf(esc_attr__('No %s found', $this->textdomain), $this->plural),
				'not_found_in_trash' => sprintf(esc_attr__('No %s found in Trash', $this->textdomain), $this->plural),
				'parent_item_colon' => sprintf(esc_attr__('Parent %s:', $this->textdomain), $this->singular),
				'title' => sprintf(esc_attr__('Enter %s title here', $this->textdomain), $this->singular),
			];
			...
			if ($postType == $this->postTypeName) {
				...
				foreach ($filters as $taxonomy_name) {
					...
					if ($terms) {
						...
						// default show all
						printf('<option value="0">%s</option>', sprintf(esc_attr__('Show all %s', $this->textdomain), $tax->label));
					...
					if ($key === $after) {
						...
						foreach ($this->taxonomies as $taxonomy) {
							...
							if ($taxonomy !== 'category' && $taxonomy !== 'post_tag') {
								...
								$newColumns[$taxonomy] = sprintf(esc_attr__('%s', $this->textdomain), $label);
							}
						}
					}
				}
			...
			switch ($column) {
				...
				case taxonomy_exists($column):
					...
					// if no terms found.
						...
						// echo no terms.
						printf(esc_attr__('No %s', $this->textdomain), $taxonomy_object->labels->name);
					}
					break;
		...
		public function modifyUpdatedMessages($messages)
		{
			...
			if (isset($_GET['revision'])) {
				...
				$message = esc_attr__('%2$s restored to revision from %1$s', $this->textdomain);
				$revision = sprintf($message, $revision_title, $singular);
			}
	
			$messages[$this->postTypeName] = [
				0 => '',
				1 => sprintf(esc_attr__('%s updated.', $this->textdomain), $singular),
				2 => esc_attr__('Custom field updated.', $this->textdomain),
				3 => esc_attr__('Custom field deleted.', $this->textdomain),
				4 => sprintf(esc_attr__('%s updated.', $this->textdomain), $singular),
				5 => $revision,
				6 => sprintf(esc_attr__('%s updated.', $this->textdomain), $singular),
				7 => sprintf(esc_attr__('%s saved.', $this->textdomain), $singular),
				8 => sprintf(esc_attr__('%s submitted.', $this->textdomain), $singular),
				9 => sprintf(
					esc_attr__('%2$s scheduled for: <strong>%1$s</strong>.', $this->textdomain),
					date_i18n(esc_attr__('M j, Y @ G:i', $this->textdomain), strtotime($post->post_date)),
					$singular
				),
				10 => sprintf(esc_attr__('%s draft updated.', $this->textdomain), $singular),
			];
